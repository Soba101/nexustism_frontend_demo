...
This frontend integrates with the backend API (default port 8001) for real ticket, analytics, and causal graph data.
...
- `LoginPage`: Authentication UI (Supabase-backed)
...
**Data Sources**: All ticket, analytics, and causal graph data is fetched from the backend via React Query hooks in `src/services/api.ts`.
...
The frontend displays the model outputs via API responses.
...
All API access goes through `src/services/api.ts` and the `fetchAPI<T>()` helper. The backend is expected to run on port 8001 (per PRD).
